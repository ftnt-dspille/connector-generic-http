...
- [TL;DR: which fields do I actually need?](#tldr-which-fields-do-i-actually-need)
- [How vendor auth translates to connector fields](#how-vendor-auth-translates-to-connector-fields)
  - [Static API key in a header](#static-api-key-in-a-header)
  - [Static API key as a query parameter](#static-api-key-as-a-query-parameter)
  - [Bearer token (static, long-lived)](#bearer-token-static-long-lived)
  - [OAuth2 client credentials (machine-to-machine)](#oauth2-client-credentials-machine-to-machine)
  - [Username/password login, then JWT on subsequent calls](#usernamepassword-login-then-jwt-on-subsequent-calls)
  - [API key sent on login request as a header, returning a short-lived JWT](#api-key-sent-on-login-request-as-a-header-returning-a-short-lived-jwt)
  - [Plain string token in login response body (filebrowser)](#plain-string-token-in-login-response-body-filebrowser)
- [Configuration (connection-level)](#configuration-connection-level)
  - [Connection target](#connection-target)
  - [Authentication Type](#authentication-type)
  - [Headers, timeout, retry](#headers-timeout-retry)
- [Actions](#actions)
  - [Common request parameters (most actions share these)](#common-request-parameters-most-actions-share-these)
  - [Action picker](#action-picker)
- [Pagination](#pagination)
- [Upload File action](#upload-file-action)
- [Download File action](#download-file-action)
- [Scheduled Data Ingestion](#scheduled-data-ingestion)
...
| Call an API with a static token                                 | `server_url`, `auth_type=Bearer Token`, `bearer_token`                                             | N/A                  |
| Call an API behind login (FortiSOAR-style `/auth/authenticate`) | `auth_type=Token Login` + the Login fields                                                         | N/A                  |
| Call an OAuth2 client-credentials API                           | `auth_type=OAuth2 Client Credentials` + token URL / client id / secret                             | N/A                  |
| Hit two APIs needing different auth                             | Create a separate connector configuration per API and pick the right one in each action            | N/A                  |
| Walk paginated endpoints                                        | Use **HTTP Paginate** action (not GET in a loop)                                                   | N/A                  |
| Upload an FSR attachment somewhere                              | Use **Upload File** action with `attachment_id`                                                    | N/A                  |
| Download a remote file into FSR                                 | Use **Download File** action with `rest_api`                                                       | N/A                  |
...
Why a dedicated auth type and not Default Custom Headers? Because `api_key` is stored encrypted. Default Custom Headers is plaintext, so don't put secrets there.
...
The connector handles the token fetch and caches it until expiry, so you don't write the two-step flow.

---

### Username/password login, then JWT on subsequent calls
...
### API key sent on login request as a header, returning a short-lived JWT
...
|------------------------|-------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `login_url`            | N/A                     | Where to POST. Relative paths join Server URL.                                                                                                                             |
...
| `login_username` / `login_password` | N/A        | The credentials. Only required for `json` body type.                                                                                                                        |
...
Why `login_username_field` exists: vendors can't agree on the body key name. `{"username":"…"}`, `{"email":"…"}`, `{"user":"…"}` are all common, so this lets you match what the API documents without touching code.
...
Why `login_token_path` is blank: normally you'd dot-walk into `{"access_token": "…"}` to pluck the value. When the entire response body is the token string (`eyJhbG…`), there's nothing to walk into, so blank means "use the body as-is".
...
> **Multiple APIs with different auth in one playbook?** Create a separate connector configuration per API. Auth is connection-level by design, and there is no per-call auth override.
...
The reverse of Upload File. GETs a remote URL and either creates an FSR Attachment or drops the file on the integration agent. Streams to disk, so the response body never goes through the playbook engine.
...
**Reducing the path with `response_path`:** set it on the action step and `data.body` becomes the hoisted value rather than the full response body. If `response_path=observables`, then `data.body` is the observables list directly, and `data.body[0]` is the first record.
